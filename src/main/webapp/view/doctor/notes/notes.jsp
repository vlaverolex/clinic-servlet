    ...
    <div class="container">
        ...
        <div class="section-show-all__add-button">
            <a class="btn btn-primary"
               href="${pageContext.request.contextPath}/app/doctor/patients/medical-book/create?patientId=${requestScope.patient.id}"><fmt:message
                    key="newNote" bundle="${messages}"/></a>
        </div>

        <div class="section__inner section-show-all">
            <h2 class="section__title section-show-all__title"><fmt:message key="medicalBook"
                                                                            bundle="${messages}"/></h2>
                    ...
                    <tr>
                        ...
                        <th scope="row">
                            <a class="btn btn-success medical-note-view-link"
                               href="${pageContext.request.contextPath}/app/doctor/patients/medical-book/note?patientId=${note.patientId}&noteId=${note.id}"><fmt:message
                                    key="view" bundle="${messages}"/>
                                <c:if test="${note.surgery!=null && note.surgeryDone==false ||
                                (note.procedures!=null && note.proceduresDone==false)}">
        ...
        <jsp:include page="/view/parts/pager.jsp">
            <jsp:param name="sortParam"
                       value="column=${requestScope.pager.sorting.column}&direction=${requestScope.pager.sorting.direction}&patientId=${requestScope.patient.id}"/>
        </jsp:include>
    </div>
</section>
</body>
