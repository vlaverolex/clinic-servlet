...
<section class="section section-show-all">
    <div class="container">
        <div class="section__inner section-show-all">
            <div>
                <a style="text-transform: capitalize !important;" class="btn btn-info back-button" href="${pageContext.request.contextPath}/app/nurse/patients/medical-book?patientId=${requestScope.patient.id}"><fmt:message key="back" bundle="${messages}"/></a>
            </div>

            <h2 class="section__title section-show-all__title"><fmt:message key="note" bundle="${messages}"/></h2>

            <div class="note-item"><fmt:message key="username" bundle="${messages}"/>: <span>${requestScope.patient.username}</span></div>
            ...
            <form method="post"
                  action="${pageContext.request.contextPath}/app/nurse/patients/medical-book/note/make-procedures?noteId=${requestScope.note.id}&patientId=${requestScope.note.patientId}&executorId=${requestScope.currentUser.id}">
