    ...
    <div class="container">
        <div>
            <a style="text-transform: capitalize !important;" class="btn btn-info back-button"
               href="${pageContext.request.contextPath}/app/admin/patients"><fmt:message key="back"
                                                                                         bundle="${messages}"/></a>
        </div>
        <div class="row">
            <div class="col-md-6 col-md-offset-3">
                <div class="panel panel-default" style="margin-top:45px">
                    <div class="panel-heading">
                        <h3 class="panel-title"><fmt:message key="addingPatient" bundle="${messages}"/></h3>
                    </div>
                    <div class="panel-body">
                        ...
                        <form action="${pageContext.request.contextPath}/app/admin/patients/create" method="post">
                            <input name="${_csrf.parameterName}" value="${_csrf.token}" type="hidden">
                            <div class="form-group">
                                <label for="username"><fmt:message key="username" bundle="${messages}"/>name</label>
                            ...
                            <div class="form-group">
                                <label for="doctorUsername"><fmt:message key="doctor" bundle="${messages}"/></label>
                                <label>
                                    <select name="doctorUsername" id="doctorUsername">
                                        <c:forEach items="${requestScope.doctorsGroupedByCategories}"
                                                   var="doctorsGroupedByCategories">
                                            <optgroup
                                                    label="<fmt:message key="${doctorsGroupedByCategories.key.engName()}" bundle="${messages}"/>">
                                                <c:forEach items="${doctorsGroupedByCategories.value}" var="doctor">
                                                    <option name="doctorUsername">${doctor.username}</option>
                                                </c:forEach>
                                            </optgroup>
                                        </c:forEach>
                                    </select>
