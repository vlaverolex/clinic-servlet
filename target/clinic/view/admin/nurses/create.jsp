...
<section class="section">
    <div class="container">
        <div>
            <a style="text-transform: capitalize !important;" class="btn btn-info back-button" href="${pageContext.request.contextPath}/app/admin/nurses"><fmt:message key="back" bundle="${messages}"/></a>
        </div>
        <div class="row">
            <div class="col-md-6 col-md-offset-3">
                <div class="panel panel-default" style="margin-top:45px">
                    <div class="panel-heading">
                        <h3 class="panel-title"><fmt:message key="addingNurse" bundle="${messages}"/></h3>
                    </div>
                    <div class="panel-body">

                        <c:if test="${requestScope.userExistErrorMessage != null}">
                            <div class="alert alert-danger"
                                 role="alert"><fmt:message key="${requestScope.userExistErrorMessage}" bundle="${messages}"/></div>
                        </c:if>
                        <c:if test="${requestScope.usernameLengthErrorMessage != null}">
                            <div class="alert alert-danger"
                                 role="alert"><fmt:message key="${requestScope.usernameLengthErrorMessage}" bundle="${messages}"/></div>
                        </c:if>
                        <c:if test="${requestScope.passwordLengthErrorMessage != null}">
                            <div class="alert alert-danger"
                                 role="alert"><fmt:message key="${requestScope.passwordLengthErrorMessage}" bundle="${messages}"/></div>
                        </c:if>
                        <c:if test="${requestScope.passwordsDoNotMatch != null}">
                            <div class="alert alert-danger"
                                 role="alert"><fmt:message key="${requestScope.passwordsDoNotMatch}" bundle="${messages}"/></div>
                        </c:if>

                        <form action="${pageContext.request.contextPath}/app/admin/nurses/create" method="post">
                            ...
                            <div class="form-group">
                                <label for="confirmPassword"><fmt:message key="confirmPassword" bundle="${messages}"/></label>
